...
def is_whitelisted():
    async def predicate(interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "❌ Access Denied: This command is restricted to the Bot Owner or Security Role.",
            ephemeral=True,
        )
        return False
...
class EmergencyPanelView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        emoji="🔒",
        custom_id="persistent_lock_all_nuke",
    )
    async def lock_all(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if interaction.user.id not in WHITELISTED_USERS and not any(
            r.id == ROLE_SECURITY for r in interaction.user.roles
        ):
            await interaction.response.send_message(
        ...
        await interaction.response.edit_message(
            embed=embed, view=MainDashboardView(self.cog)
        )
        await interaction.followup.send(
            "🔒 Successfully locked and disabled **all** security systems!",
            ephemeral=True,
        )

    @discord.ui.button(
        label="UNLOCK ALL NUKE",
        style=discord.ButtonStyle.success,
        emoji="🔓",
        custom_id="persistent_unlock_all_nuke",
    )
    async def unlock_all(
        ...
        await interaction.followup.send(
            "🔓 Successfully unlocked and enabled **all** security systems!",
            ephemeral=True,
        )
    ...
    @discord.ui.button(
        ...
        row=1,
        custom_id="persistent_back_home_emg",
    )
    async def back_home(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
...
class AntiSystemSelect(discord.ui.Select):
        ...
        for key in self.cog.ANTI_SETTINGS.keys():
            ...
            options.append(
                discord.SelectOption(
                    label=key, value=key, description=desc, emoji=emoji
                )
            )

        super().__init__(
            placeholder="Select a system to toggle...",
            min_values=1,
            max_values=1,
            options=options,
            custom_id="persistent_anti_system_select",
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        new_status = (
            "Enabled 🟢"
            if self.cog.ANTI_SETTINGS[selected_key]
            else "Disabled 🔴"
        )
        view = IndividualControlView(self.cog)
        await interaction.response.edit_message(view=view)
        await interaction.followup.send(
            f"⚙️ Status of system `{selected_key}` changed to: **{new_status}**",
            ephemeral=True,
        )
...
class IndividualControlView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        row=1,
        custom_id="persistent_back_home_ind",
    )
    async def back_home(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
...
class MainDashboardView(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.danger,
        emoji="⚡",
        custom_id="persistent_btn_emergency",
        row=0,
    )
    async def btn_emergency(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if interaction.user.id not in WHITELISTED_USERS and not any(
            r.id == ROLE_SECURITY for r in interaction.user.roles
        ...
        await interaction.response.edit_message(
            embed=embed, view=EmergencyPanelView(self.cog)
        )

    @discord.ui.button(
        label="⚙️ Individual Control",
        style=discord.ButtonStyle.primary,
        emoji="🎛️",
        custom_id="persistent_btn_individual",
        row=0,
    )
    async def btn_individual(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if interaction.user.id not in WHITELISTED_USERS and not any(
            r.id == ROLE_SECURITY for r in interaction.user.roles
        ...
        await interaction.response.edit_message(
            embed=embed, view=IndividualControlView(self.cog)
        )

    @discord.ui.button(
        label="📊 Anti Settings Status",
        style=discord.ButtonStyle.success,
        emoji="📋",
        custom_id="persistent_btn_anti_settings",
        row=1,
    )
    async def btn_anti_settings(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        if interaction.user.id not in WHITELISTED_USERS and not any(
            r.id == ROLE_SECURITY for r in interaction.user.roles
        ...
        back_btn = discord.ui.Button(
            label="🔙 Back",
            style=discord.ButtonStyle.secondary,
            custom_id="persistent_back_btn_status",
        )

        async def back_callback(inter):
            await inter.response.edit_message(
                embed=self.cog.get_main_panel_embed(),
                view=MainDashboardView(self.cog),
            )
...
class Security(commands.Cog):
    ...
    def save_words(self):
        try:
            with open(self.words_file, "w", encoding="utf-8") as f:
                json.dump(
                    list(self.filtered_words), f, ensure_ascii=False, indent=4
                )
    ...
    async def safe_warn(self, message: discord.Message, text: str):
        try:
            await message.channel.send(
                f"⚠️ {message.author.mention} {text}", delete_after=5
            )
    ...
    async def on_message(self, message):
        try:
            ...
            if not is_allowed and (
                self.protection_enabled("antilink")
                or self.protection_enabled("antiinvite")
            ):
                ...
                if re.search(link_pattern, message.content, re.IGNORECASE):
                    ...
                    await self.safe_warn(
                        message,
                        "Links and Discord invites are not allowed here.",
                    )
                    return

            if not is_allowed and (
                self.protection_enabled("antimention")
                or self.protection_enabled("antiping")
            ):
                mentions = len(message.mentions) + len(message.role_mentions)
                everyone_ping = message.mention_everyone
                if mentions > self.MAX_MENTIONS or (
                    everyone_ping
                    and (
                        self.protection_enabled("antiping")
                        or self.protection_enabled("antieveryone")
                    )
                ):
                    await self.safe_delete_message(message)
                    await self.safe_warn(
                        message, "Excessive mentions/pings are blocked."
                    )
                    return
            ...
            if not is_allowed and self.protection_enabled("antiemoji"):
                if self.count_emojis(message.content) > self.MAX_EMOJIS:
                    await self.safe_delete_message(message)
                    await self.safe_warn(
                        message, "Too many emojis in this message."
                    )
                    return

            if (
                not is_allowed
                and self.protection_enabled("antispamfiles")
                ...
                for attachment in message.attachments:
                    if (
                        ...
                        await self.safe_warn(
                            message,
                            f"File size exceeds the allowed limit ({self.MAX_FILE_SIZE_MB}MB).",
                        )
                        return
            ...
            if not is_allowed and self.protection_enabled("anticaps"):
                ...
                if len(letters) >= 8:
                    ...
                    if upper / len(letters) >= self.CAPS_PERCENT:
                        ...
                        await self.safe_warn(
                            message,
                            "Excessive use of uppercase letters is not allowed.",
                        )
                        return

            if not is_allowed and (
                self.protection_enabled("antispam")
                or self.protection_enabled("antiflood")
            ):
                ...
                if self.protection_enabled("antispam"):
                    recent = [
                        t for t in history if now - t <= self.SPAM_WINDOW
                    ]
                    if len(recent) >= self.SPAM_LIMIT:
                        ...
                        await self.safe_delete_message(message)
                        await self.safe_warn(
                            message, "You have been timed out for spamming."
                        )
                        history.clear()
                        return
                ...
                if self.protection_enabled("antiflood"):
                    recent = [
                        t for t in history if now - t <= self.FLOOD_WINDOW
                    ]
                    if len(recent) >= self.FLOOD_LIMIT:
                        await self.safe_delete_message(message)
                        await self.safe_warn(
                            message,
                            "Please do not send messages too quickly.",
                        )
                        return

            if (
                not is_allowed
                and self.protection_enabled("badwords")
                and self.filtered_words
            ):
                ...
                if matched:
                    ...
                    self.word_warning_counts[user_id] = (
                        self.word_warning_counts.get(user_id, 0) + 1
                    )
                    ...
                    if count == 1:
                        await self.safe_warn(
                            message,
                            "You used a prohibited word! This is your **1st Warning**.",
                        )
                    elif count == 2:
                        await self.safe_warn(
                            message,
                            "You used a prohibited word again! This is your **2nd Warning**.",
                        )
                    else:
                        try:
                            await message.author.timeout(
                                datetime.timedelta(days=1),
                                reason="Reached 3 warnings for using bad words",
                            )
                            self.word_warning_counts[user_id] = 0
                            await self.safe_warn(
                                message,
                                "You reached **3 warnings** for using prohibited words and have been muted for **1 Day**.",
                            )
                        except Exception as e:
                            print(
                                f"[ANTI] Failed to timeout user for bad words: {e}"
                            )
                    return
    ...
    async def on_member_join(self, member: discord.Member):
        try:
            if (
                not self.protection_enabled("antibot")
                or not member.bot
                or self.is_protected_member(member)
            ):
                return
        ...
        try:
            ...
            if entry and entry.target:
                ...
                if webhook_id:
                    ...
                    if webhook.user and not self.is_protected_member(
                        webhook.user
                    ):
                        await webhook.delete(reason="AntiWebhook")
                        await self.punish_executor(
                            guild,
                            entry.user,
                            "AntiWebhook: unauthorized webhook creation",
                        )
        except Exception:
            traceback.print_exc()
    ...
    async def on_guild_channel_delete(
        ...
        try:
            if not (
                self.protection_enabled("antichannel")
                or self.protection_enabled("antichanneldelete")
            ):
                return
            entry = await self.get_recent_audit_entry(
                channel.guild,
            ...
            if entry and entry.user and not self.is_protected_member(entry.user):
                if isinstance(entry.user, discord.Member):
                    await channel.guild.kick(
                        entry.user, reason="AntiChannel: deleted a channel"
                    )
        except Exception:
            traceback.print_exc()
    ...
    async def on_guild_role_delete(self, role: discord.Role):
        try:
            if not (
                self.protection_enabled("antiroledelete")
                or self.protection_enabled("antirole")
            ):
                return
            entry = await self.get_recent_audit_entry(
            ...
            if entry and entry.user and not self.is_protected_member(entry.user):
                ...
                await self.punish_executor(
                    role.guild,
                    entry.user,
                    "AntiRole: unauthorized role deletion",
                )
        except Exception:
            traceback.print_exc()

    @app_commands.command(
        name="controlpanel", description="Open the server main security control panel"
    )
    @is_whitelisted()
    ...
    @app_commands.command(
        name="antisettings", description="Display security settings statuses"
    )
    @is_whitelisted()
    async def antisettings(self, interaction: discord.Interaction):
        embed = discord.Embed(
            title="🛡️ Security Settings Panel", color=discord.Color.blurple()
        )
        for k, v in self.ANTI_SETTINGS.items():
            embed.add_field(
                name=k, value="🟢 Enabled" if v else "🔴 Disabled", inline=True
            )
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @app_commands.command(
        name="removeword", description="Add a word to the bad words list"
    )
    ...
    async def removeword(self, interaction: discord.Interaction, word: str):
        ...
        if w in self.filtered_words:
            await interaction.response.send_message(
                f"⚠️ The word `{w}` is already in the ban list.", ephemeral=True
            )
            return
        self.filtered_words.add(w)
        self.save_words()
        await interaction.response.send_message(
            f"✅ Successfully added the word `{w}` to the ban list and saved.",
            ephemeral=True,
        )

    @app_commands.command(
        name="showwords", description="Display the current bad words list"
    )
    @is_whitelisted()
    async def showwords(self, interaction: discord.Interaction):
        if not self.filtered_words:
            await interaction.response.send_message(
                "📂 The bad words list is currently empty.", ephemeral=True
            )
            return
    ...
    @app_commands.command(
        name="delword", description="Remove a word from the ban list"
    )
    ...
    async def delword(self, interaction: discord.Interaction, word: str):
        ...
        if w not in self.filtered_words:
            await interaction.response.send_message(
                f"❌ The word `{w}` is not in the ban list.", ephemeral=True
            )
            return
        self.filtered_words.remove(w)
        self.save_words()
        await interaction.response.send_message(
            f"🗑️ Successfully removed the word `{w}` from the ban list.",
            ephemeral=True,
        )
...
async def main():
    async with bot:
        ...
        if not TOKEN:
            print(
                "❌ Error: Please set the 'TOKEN' environment variable in Railway."
            )
            return
